...
class TorcsEnv:
    ...
    default_speed = 50    #used in automatic speed control
       ...
       #print("Step")
        ...
        if self.throttle is False:
            ...
            if client.S.d['speedX'] < 10:
                client.R.d['accel'] += .005/(client.S.d['speedX']+.1)
        ...
        speed_over = sp - speed_limit
        if speed_over > 0:
            print('speed over!')

        progress = sp*np.cos(obs['angle']) - np.abs(sp*np.sin(obs['angle'])) - sp * np.abs(obs['trackPos']) - max(0, speed_over)
        reward = progress
